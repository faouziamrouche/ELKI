...
def main(argv):
	...
	for x in range(0,repet):
		...
		for i in algos:
			...
			df.columns = [i]
			# df = df.replace(['cof-outlier=','abod-outlier','inflo-outlier','knn-outlier','knnw-outlier','ldof-outlier','lof-outlier','loop-outlier','odin','simplified-lof-outlier','kdeos-outlier'],'')
			# df = df.replace(regex=True, to_replace=["cof-outlier=","abod-outlier=","inflo-outlier"], value="")
			li.append(df)
		frame = pd.concat(li, axis=1)#, ignore_index=True)
		# frame = frame.replace(['cof-outlier=','abod-outlier','inflo-outlier','knn-outlier','knnw-outlier','ldof-outlier','lof-outlier','loop-outlier','odin','simplified-lof-outlier','kdeos-outlier'],'')
